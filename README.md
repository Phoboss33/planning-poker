...
1. Скопируйте пример переменных окружения и при необходимости обновите значения:
   ...
   - Клиент берёт адрес из `VUE_APP_SERVER`, если он задан и не содержит `localhost`. Иначе он возьмёт хост текущей страницы и его порт (если указан). Если фронтенд и бэкенд на разных хостах/портах, обязательно пропишите точный URL бэкенда в `VUE_APP_SERVER`.
...
2. Склонируйте репозиторий и подготовьте переменные окружения:
   ...
   В `client/.env` задайте `VUE_APP_SERVER` только если бэкенд на другом хосте/порте. Если фронтенд и бэкенд на одном хосте, оставьте переменную пустой — клиент сам возьмёт текущий хост и порт страницы (если он указан). Иначе пропишите публичный URL бэкенда (например, `http://147.45.183.5:3000` или адрес reverse‑proxy), чтобы исключить обращения к `localhost` после сборки.
